...
async def check_routing(state: LawState) -> dict:
    ...
    needs_tax = any(kw in question_lower for kw in ["tax", "irs", "thuế", "evasion", "fraud", "penalty"])
    needs_compliance = any(kw in question_lower for kw in ["compliance", "sec", "regulation", "sox", "aml", "fcpa"])
    
    logger.info("Routing decision (optimized rule-based): needs_tax=%s needs_compliance=%s", needs_tax, needs_compliance)
